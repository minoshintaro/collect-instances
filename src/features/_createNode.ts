...
// export function createText(props: ElementProp): TextNode {
...
export function createFrame(props: ElementProp): FrameNode {
  const { name, layout, theme } = props;
  ...
  if (theme) {
    const { fill, radius } = theme;
    newFrame.fills = theme.fill ? theme.fill[0] : [];
    newFrame.cornerRadius = theme.radius ? theme.radius : 0;
  }
